...
3. **Fill in the offering** — name, service_type, capabilities, upstream_access_config, payout_price. Cross-check against `unitysvc-sellers/docs/file-schemas.md` for field semantics. **`capabilities` is a platform vocabulary, not free text** — pick from the list in [file-schemas.md § Capabilities](https://unitysvc-sellers.readthedocs.io/en/latest/file-schemas/#capabilities) (`chat`, `embed`, `http-relay`, `deliver-message`, …). An invented value validates but has no topic and splits the catalog's capability filter. Attributes that merely *qualify* the capability — `vision`, `tools`, context size — go in `details`, never in `capabilities`. **For secret references:** put sensitive values behind `${ customer_secrets.<NAME> }` with a service-specific prefix (`SMTP_RELAY_HOST`, not bare `SMTP_HOST`); put operational config in direct params or as literal fields with `?? default` fallbacks. See **§11**.
