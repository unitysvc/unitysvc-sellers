"""Flat ``specs/`` layout helpers + the ``validate`` command.

The ``specs`` command group itself lives in ``specs.py``, which registers the
``validate`` defined here. This module holds the layout rules and the folder
discovery/validation helpers (callback-free, so it can be unit-tested in
isolation).
