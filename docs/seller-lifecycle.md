...
### Submitting for review vs. marking pending
...
| **Mark pending** | `usvc_seller services mark-pending <name>` | A pure status change (draft/rejected/suspended → `pending`) with **no** tests. Makes the service routable so you can test its code examples on-wire while you iterate. |
...
usvc_seller services mark-pending my-provider/my-service
...
!!! note "SDK equivalents"
    `client.services.get(id).mark_pending()` (pure status change) and
    `client.services.get(id).submit()` (submit for review + tests), or the
    manager forms `client.services.update(id, {"status": "pending"})` and
