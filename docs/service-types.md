...
You don't author a pool service by hand. Membership comes **only** from instantiating a pool-named [service template](service-templates.md) — `usvc seller specs upload` of a hand-written spec always produces a plain standalone service, never a pool member. Opting in is deliberately simple: instantiate the pool template (dashboard or `usvc seller params instantiate`) and supply just your upstream URL (and a key secret, if your upstream needs one).
